...
export function ParticipantExperience() {
  ...
  async function submit() {
    ...
    try {
      const response = await fetch("/api/tiles", {
        ...
        body: JSON.stringify({ ...draft, room: ROOM, clientId: getClientId() }),
      });
  ...
  async function capturePoint() {
    ...
    if (cameraOn && videoRef.current?.videoWidth) {
      ...
      await new Promise<void>((resolve) => {
        ...
        image.src = "/identity-map-field.png";
      });
      if (image.width) drawCover(ctx, image, image.width, image.height, canvas.width, canvas.height);
      else { ctx.fillStyle = "#f2e8da"; ctx.fillRect(0, 0, canvas.width, canvas.height); }
    }

    const shade = ctx.createLinearGradient(0, 0, 0, canvas.height);
    shade.addColorStop(0, "rgba(59,43,41,.18)");
    shade.addColorStop(.46, "rgba(242,232,218,.08)");
    shade.addColorStop(1, "rgba(59,43,41,.62)");
    ctx.fillStyle = shade;
    ...
    drawWave(ctx, draft.currentPhrase || "מי שאני עכשיו", 430, 34, "rgba(255,244,232,.96)", "rgba(59,43,41,.72)", canvas.width);
    drawWave(ctx, draft.futurePhrase || "מי שאני בוחרת לקרב", 720, 46, "#6d3f00", "rgba(255,231,180,.92)", canvas.width);
  ...
  async function shareCapture() {
    ...
    if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
      try { await navigator.share({ files: [file], title: "המרחק ביני לביני", text: "נקודה בזמן, קבוצת האינטגרציה" }); return; } catch {}
    }
    downloadCapture();
    setNotice("התמונה נשמרה. עכשיו אפשר לצרף אותה לקבוצת האינטגרציה.");
  }
